...
export function fetchUrl(param: fetchParam): Promise<string> {
...
export function createHeaderListener(type: string) {
  ...
  result.push(type);
  // @ts-ignore
  if (IS_CHROME && chrome.webRequest.OnBeforeSendHeadersOptions && chrome.webRequest.OnBeforeSendHeadersOptions.hasOwnProperty('EXTRA_HEADERS')) {
